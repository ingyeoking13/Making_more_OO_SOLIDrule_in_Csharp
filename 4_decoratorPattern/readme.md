...
`decorator pattern`을 설명하기 위해 `나`라는 객체를 표현해본다고 하자.  
`나`를 표현하기 위해 클래스를 하나 생성 해 보았다.  
`나`를 일반화 하여 표현하기 위해, `사람`이라는 클래스를 생성하였다.  
이름과 자기 소개 메서드가 있다.  
...
첫번째 시도가 썩 좋지 않은 결과로 이어질 수 있다는 건 확인하였다.  
...
하지만 두번째처럼 성격을 리스트 처럼 가지고 있는 것 또한 좋은 선택이다.   
좋고 나쁨의 판단은 프로젝트가 어떻게 확장 되느냐에 따라, 리팩토링 시점에 달린 것 아닐까.   

어쨌든,
위 트리에서 C 클래스는 A 클래스 필드를 가지고 있기만 하면 어떤 B 타입에 대해 가르킬 수 있다.    
그리고 계속해서 C 클래스를 덧댈 수 있는 것이다.  

책에서는 위 클래스 트리 보다 쉬운 클래스 개요를 썼다. 원리는 같다.    
:Smile:
